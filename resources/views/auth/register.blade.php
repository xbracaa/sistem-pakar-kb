...
<body class="bg-[#F5F3E9] min-h-screen flex items-center justify-center p-6">
    <div class="w-full max-w-md bg-white rounded-3xl shadow-xl border border-[#E8E5DA] p-8 md:p-10 mt-8 mb-8">
        <div class="mb-6">
        ...
        <div class="flex flex-col items-center mb-8">
            ...
            <h1 class="text-2xl font-extrabold text-brand-dark tracking-tight">Daftar Akun</h1>
        ...
        <form method="POST" action="{{ route('register.post') }}" class="space-y-5">
            @csrf
            <div>
                <label class="block text-sm font-bold text-brand-dark mb-2">Nama Lengkap</label>
                <input type="text" name="name" value="{{ old('name') }}" class="w-full bg-[#F5F8F6] border border-[#D0E3D6] rounded-xl px-4 py-3 text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-dark/20 transition-all font-medium" required autofocus>
            </div>
            <div>
            ...
            <div>
                <label class="block text-sm font-bold text-brand-dark mb-2">Konfirmasi Password</label>
                <input type="password" name="password_confirmation" class="w-full bg-[#F5F8F6] border border-[#D0E3D6] rounded-xl px-4 py-3 text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-dark/20 transition-all font-medium" required>
            </div>
            ...
            <div class="text-center mt-6">
                <p class="text-sm text-brand-accent font-medium">Sudah punya akun? <a href="{{ route('login') }}" class="text-brand-dark font-bold hover:underline">Masuk</a></p>
            </div>
        </form>
    </div>
</body>
