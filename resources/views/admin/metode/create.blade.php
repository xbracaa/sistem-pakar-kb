...
<div class="bg-gradient-to-br from-white to-[#F5F8F6] rounded-3xl border border-white p-8 shadow-lg shadow-brand-dark/5 max-w-2xl relative">
    ...
    <form action="{{ route('admin.metode.store') }}" method="POST">
        ...
        <div class="mb-6 relative z-10">
            ...
            <textarea name="kelebihan" rows="3" class="w-full bg-white border border-[#D0E3D6] rounded-xl px-4 py-3 text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-dark/20 font-medium leading-relaxed transition-all shadow-sm hover:shadow-md">{{ old('kelebihan') }}</textarea>
        </div>

        <div class="mb-8 relative z-10">
            <label class="block text-[10px] font-bold tracking-widest text-brand-accent uppercase mb-2">Kekurangan KB</label>
            <textarea name="kekurangan" rows="3" class="w-full bg-white border border-[#D0E3D6] rounded-xl px-4 py-3 text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-dark/20 font-medium leading-relaxed transition-all shadow-sm hover:shadow-md">{{ old('kekurangan') }}</textarea>
        </div>
