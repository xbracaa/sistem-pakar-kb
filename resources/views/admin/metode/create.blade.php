...
<div class="bg-white rounded-3xl border border-brand-border p-8 shadow-sm max-w-xl">
    ...
    <form action="{{ route('admin.metode.store') }}" method="POST">
        ...
        <div class="mb-8">
            ...
            <input type="text" name="nama_metode" value="{{ old('nama_metode') }}" class="w-full bg-[#F5F8F6] border border-[#D0E3D6] rounded-xl px-4 py-3 text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-dark/20 font-bold" placeholder="Misal: KB Suntik 1 Bulan" required>
        </div>

        <div class="mb-6">
            <label class="block text-sm font-bold text-brand-dark mb-2">Kelebihan KB</label>
            <textarea name="kelebihan" rows="3" class="w-full bg-[#F5F8F6] border border-[#D0E3D6] rounded-xl px-4 py-3 text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-dark/20 font-medium leading-relaxed">{{ old('kelebihan') }}</textarea>
        </div>

        <div class="mb-8">
            <label class="block text-sm font-bold text-brand-dark mb-2">Kekurangan KB</label>
            <textarea name="kekurangan" rows="3" class="w-full bg-[#F5F8F6] border border-[#D0E3D6] rounded-xl px-4 py-3 text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-dark/20 font-medium leading-relaxed">{{ old('kekurangan') }}</textarea>
        </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="bg-brand-dark text-white px-8 py-3 rounded-xl font-bold hover:bg-opacity-90 shadow-lg transition-all">Simpan Metode</button>
        </div>
    </form>
</div>
@endsection
