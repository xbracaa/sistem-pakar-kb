...
<div class="bg-white rounded-3xl border border-brand-border p-8 shadow-sm max-w-xl">
    ...
    <form action="{{ route('admin.kondisi.store') }}" method="POST">
        ...
        <div class="mb-8">
            ...
            <input type="text" name="nama_kondisi" value="{{ old('nama_kondisi') }}" class="w-full bg-[#F5F8F6] border border-[#D0E3D6] rounded-xl px-4 py-3 text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-dark/20 font-bold" placeholder="Misal: Kista Ovarium" required>
        </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="bg-brand-dark text-white px-8 py-3 rounded-xl font-bold hover:bg-opacity-90 shadow-lg transition-all">Simpan Kondisi</button>
        </div>
    </form>
</div>
@endsection
