...
<div class="bg-gradient-to-br from-white to-[#F5F8F6] rounded-3xl border border-white p-8 shadow-lg shadow-brand-dark/5 max-w-2xl relative">
    ...
    <form action="{{ route('admin.kondisi.update', $kondisi->id) }}" method="POST">
        ...
        <div class="mb-8 relative z-10">
            ...
            <input type="text" name="nama_kondisi" value="{{ old('nama_kondisi', $kondisi->nama_kondisi) }}" class="w-full bg-white border border-[#D0E3D6] rounded-xl px-4 py-3 text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-dark/20 font-bold transition-all shadow-sm hover:shadow-md" required>
        </div>
